...
from .ZTransformerModel import LayerNorm, attention, clones, SublayerConnection, PositionwiseFeedForward
...
class MultiHeadedDotAttention(nn.Module):
    def __init__(self, h, d_model, dropout=0.1, scale=1, project_k_v=1, use_output_layer=1, do_aoa=0, norm_q=0, dropout_aoa=0.3):
        ...
        self.linears = clones(nn.Linear(d_model, d_model * scale), 2 + 2 * project_k_v)
    ...
    def forward(self, query, value, key, mask=None):
        ...
        if self.project_k_v == 0:
            ...
            q2_ =  self.linears[1](query).view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
            key_ = key.view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
            value_ = value.view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
        else:
            query_, q2_, key_, value_ = \
                [l(x).view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
                            for l, x in zip(self.linears, (query, query, key, value))]

        # Apply attention on all the projected vectors in batch. 
        x = attention(query_, q2_, key_, value_, mask=mask, dropout=self.dropout)
...
class ZAoAModel(AttModel):
    def __init__(self, opt):
        super(ZAoAModel, self).__init__(opt)
        self.num_layers = 2
        # mean pooling
