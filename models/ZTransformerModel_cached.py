...
def attention(query, q2, key, value, mask=None, dropout=None):
    ...
    scores1 = torch.matmul(q2, value.transpose(-2, -1)) / math.sqrt(d_k)
    scores = F.gelu(scores)
    scores1 = F.gelu(scores1)
    if mask is not None:
        ...
        scores1 = scores1.unsqueeze(-1)
        scores1 = torch.sum(scores1*mask1) / torch.sum(mask1)
    else:
        scores1 = scores1.unsqueeze(1)
        scores1 = scores1.unsqueeze(-1)
        scores1 = scores1.mean()
    att_map_pool = torch.sigmoid(scores1)
    scores = F.softmax(scores, dim=-1)
    value = torch.matmul(scores, value)
    value = F.gelu(value)
    return torch.mul(dropout(value), (1+att_map_pool))
...
class MultiHeadedAttention(nn.Module):
    def __init__(self, h, d_model, dropout=0.2):
        ...
        #self.m_k = nn.Parameter(torch.FloatTensor(1, h, h * (d_model // h)))
        #self.m_v = nn.Parameter(torch.FloatTensor(1, h, h * (d_model // h)))
    ...
    def forward(self, query, key, value, mask=None, layer_past=None):
        ...
        if layer_past is not None and layer_past.shape[2] == key.shape[1] > 1: # suppose memory size always greater than 1 
            ...
            q2 = self.linears[1](query)
            key, value = layer_past[0], layer_past[1]
            present = torch.stack([key, value])
        else:
            # 1) Do all the linear projections in batch from d_model => h x d_k 
            query, q2, key, value = \
                [l(x) for l, x in zip(self.linears, (query, query, key, value))]
        ...
        query, q2, key, value = \
            ...
            for x in [query, q2, key, value]]

        #m_k = np.sqrt(self.d_k) * self.m_k.expand(nbatches, -1, self.h, self.h * self.d_k)
        #m_v = np.sqrt(self.d_k) * self.m_v.expand(nbatches, -1, self.h, self.h * self.d_k)
 
        # 2) Apply attention on all the projected vectors in batch. 
        x = attention(query, q2, key, value, mask=mask,dropout=self.dropout)
...
class ZTransformerModel(AttModel):
    ...
    def __init__(self, opt):
        super(ZTransformerModel, self).__init__(opt)
        self.opt = opt
