...
class NTTUD_core (nn.Module):
    def __init__(self, opt, use_maxout=False):
        super(NTTUD_core, self).__init__()
        self.drop_prob_lm = opt.drop_prob_lm

        self.att_lstm = nn.LSTMCell(opt.input_encoding_size + opt.rnn_size * 2, opt.rnn_size) # we, fc, h^2_t-1
        self.att_lstm2 = nn.LSTMCell(opt.input_encoding_size + opt.rnn_size * 2, opt.rnn_size)
    ...
    def forward(self, xt, fc_feats, att_feats, p_att_feats, state, att_masks=None):
        ...
        h_att, c_att = self.att_lstm(att_lstm_input, (state[0][0], state[1][0]))
        h_att2, c_att2 = self.att_lstm2(att_lstm_input, (state[0][0], state[1][0]))
        att = self.attention(h_att, att_feats, p_att_feats, att_masks)
        att2 = self.attention2(h_att2, att_feats, p_att_feats, att_masks)
        lang_lstm_input = torch.cat([att, h_att], 1)
        lang_lstm_input2 = torch.cat([att2, h_att2], 1)
        # lang_lstm_input = torch.cat([att, F.dropout(h_att, self.drop_prob_lm, self.training)], 1) ?????
        att_gate1 = F.sigmoid(self.h2h_1(h_att+c_att))
        att_gate2 = F.sigmoid(self.h2h_1(h_att2+c_att2))
        att_gate2 = torch.add(att_gate1, att_gate1)

        h_lang, c_lang = self.lang_lstm(lang_lstm_input, (state[0][1], att_gate1*state[1][1]))
        h_lang2, c_lang2 = self.lang_lstm2(lang_lstm_input2, (state[0][1], att_gate2*state[1][1]))

        h_lang_t = torch.add(h_lang, h_lang2)
        c_lang_t = torch.add(c_lang, c_lang2)
    
        att_gate3 = F.sigmoid(self.h2h_1(h_lang_t+c_lang_t)) ##
        att_gate3 = torch.add(att_gate2, att_gate3)

        h_lang3, c_lang3 = self.lang_lstm3(lang_lstm_input+lang_lstm_input2, (h_lang_t, att_gate3*c_lang_t))
        
        output1 = F.dropout(h_lang, 0.3, self.training)
        output2 = F.dropout(h_lang2, 0.7, self.training)
        output3 = F.dropout(h_lang3, 0.8, self.training)
        output = output1 + output2 + output3
        output = F.dropout(output, 0.5, self.training)
        h_att = h_att+h_att2
        c_att = c_att+c_att2
        state = (torch.stack([h_att, h_lang3]), torch.stack([c_att, c_lang3]))

        return output, state

class NTTUD2_core (nn.Module):
    def __init__(self, opt, use_maxout=False):
        super(NTTUD2_core, self).__init__()
        self.drop_prob_lm = opt.drop_prob_lm

        self.att_lstm = nn.LSTMCell(opt.input_encoding_size + opt.rnn_size * 2, opt.rnn_size) # we, fc, h^2_t-1
        self.att_lstm2 = nn.LSTMCell(opt.input_encoding_size + opt.rnn_size * 2, opt.rnn_size)
        self.att_lstm3 = nn.LSTMCell(opt.input_encoding_size + opt.rnn_size * 2, opt.rnn_size)
        self.lang_lstm = nn.LSTMCell(opt.rnn_size * 2, opt.rnn_size) # h^1_t, \hat v
        self.lang_lstm2 = nn.LSTMCell(opt.rnn_size*2, opt.rnn_size)
        self.lang_lstm3 = nn.LSTMCell(opt.rnn_size*2, opt.rnn_size)
        self.attention = Attention(opt)
        self.attention2 = Attention(opt)
        self.attention3 = Attention(opt)
        #self.h2h_2 = nn.Linear(opt.rnn_size, opt.rnn_size)
        self.h2h_1 = nn.Linear(opt.rnn_size, opt.rnn_size)
        #self.h2h_3 = nn.Linear(opt.rnn_size, opt.rnn_size)
    ...
    def forward(self, xt, fc_feats, att_feats, p_att_feats, state, att_masks=None):
        ...
        h_att, c_att = self.att_lstm(att_lstm_input, (state[0][0], state[1][0]))
        h_att2, c_att2 = self.att_lstm2(att_lstm_input, (state[0][0], state[1][0]))
        h_att3, c_att3 = self.att_lstm3(att_lstm_input, (state[0][0], state[1][0]))
        att = self.attention(h_att, att_feats, p_att_feats, att_masks)
        att2 = self.attention2(h_att2, att_feats, p_att_feats, att_masks)
        att3 = self.attention3(h_att3, att_feats, p_att_feats, att_masks)
        lang_lstm_input = torch.cat([att, h_att], 1)
        lang_lstm_input2 = torch.cat([att2, h_att2], 1)
        lang_lstm_input3 = torch.cat([att3, h_att3], 1)
        # lang_lstm_input = torch.cat([att, F.dropout(h_att, self.drop_prob_lm, self.training)], 1) ?????
        att_gate1 = F.sigmoid(self.h2h_1(h_att+c_att))
        att_gate2 = F.sigmoid(self.h2h_1(h_att2+c_att2))
        att_gate2 = torch.add(att_gate1, att_gate1)
        att_gate3 = F.sigmoid(self.h2h_1(h_att3+c_att3)) ##
        att_gate3 = torch.add(att_gate2, att_gate3)
        h_lang, c_lang = self.lang_lstm(lang_lstm_input, (state[0][1], att_gate1*state[1][1]))
        h_lang2, c_lang2 = self.lang_lstm2(lang_lstm_input2, (state[0][1], att_gate2*state[1][1]))
        h_lang3, c_lang3 = self.lang_lstm3(lang_lstm_input3, (state[0][1], att_gate3*state[1][1]))
        h_lang_t = h_lang+h_lang2+h_lang3
        c_lang_t = c_lang+c_lang2+c_lang3
        #h_lang3, c_lang3 = self.lang_lstm3(lang_lstm_input+lang_lstm_input2, (h_lang_t, att_gate3*c_lang_t))
        output1 = F.dropout(h_lang, 0.3, self.training)
        output2 = F.dropout(h_lang2, 0.7, self.training)
        output3 = F.dropout(h_lang3, 0.8, self.training)
        output = output1 + output2 + output3
        output = F.dropout(output, 0.5, self.training)
        h_att = h_att+h_att2+h_att3
        c_att = c_att+c_att2+c_att3
        state = (torch.stack([h_att, h_lang3]), torch.stack([c_att, c_lang3]))

        return output, state


def s_attention(query, key, value):
    ...
    #scores = F.dropout(scores, 0.5, True)
    #mask = mask.view(-1, scores.size(-1)).float()
    #if mask is not None:
...
class UpDownCore_new(nn.Module):
    def __init__(self, opt, use_maxout=False):
        super(UpDownCore_new, self).__init__()
        self.drop_prob_lm = opt.drop_prob_lm

        self.att_lstm = nn.LSTMCell(opt.input_encoding_size + opt.rnn_size * 2, opt.rnn_size) # we, fc, h^2_t-1
        self.lang_lstm = nn.LSTMCell(opt.rnn_size * 2, opt.rnn_size) # h^1_t, \hat v
        self.att_lang = nn.LSTMCell(opt.rnn_size * 2, opt.rnn_size)
        self.attention = Attention(opt)
        self.attention2 = Attention(opt)
        #self.h2h_2 = nn.Linear(opt.rnn_size, opt.rnn_size)
        #self.h2h_1 = nn.Linear(opt.rnn_size, opt.rnn_size)
    ...
    def forward(self, xt, fc_feats, att_feats, p_att_feats, state, att_masks=None):
        ...
        h_att, c_att = self.att_lstm(att_lstm_input, (state[0][0], state[1][0]))
        #att_gate1 = F.sigmoid(self.h2h_1(h_att+c_att))
        #h_att = F.gelu(F.dropout(self.h2h_1(att_lstm_input), 0.5, True))
        att = self.attention(h_att, att_feats, p_att_feats, att_masks)
        #att_size = att_feats.numel() // att_feats.size(0) // att_feats.size(-1)
        #att_masks = att_masks.view(-1, att_size).float()
        #att, p_att = drop_attention(att, att, att)
        lang_lstm_input = torch.cat([att, h_att],1)
        #lang_lstm_input = F.gelu(F.dropout(self.h2h_1(lang_lstm_input), 0.5, True))
        #lang_lstm_input = torch.cat([att, F.dropout(h_att, self.drop_prob_lm, self.training)], 1) ?????
        h_lang, c_lang = self.lang_lstm(lang_lstm_input, (state[0][1], state[1][1]))
        #att, p_att = drop_attention(att, h_att, h_lang)
        #att = F.gelu(self.h2h_1(att))
        #h_att = F.dropout(h_att, 0.5, self.training)
        #h_lang = F.gelu(h_lang)
        #c_lang = F.gelu(c_lang)
        #att2 = self.attention2(h_lang, att_feats, p_att_feats, att_masks)
        #x, p_att = selfattention(att, att2, h_lang, mask=c_lang)
        att2 = self.attention2(h_lang+h_att, att_feats, p_att_feats, att_masks)
        att_lang_input = torch.cat([att2, h_lang+h_att],1)
        #h_lang = F.dropout(h_lang, 0.5, self.training)
        #h_att = F.dropout(h_att, 0.5, self.training)
        #c_lang = F.dropout(c_lang, 0.5, self.training)
        #c_att = F.dropout(c_att, 0.5, self.training)
        h_al, c_al = self.att_lang(att_lang_input, (h_att+h_lang, c_att+c_lang))
        #h_lang = F.gelu(self.h2h_1(h_lang))
        #h_att = F.gelu(self.h2h_2(h_att))
        #h_lang = s_attention(h_att, h_lang, h_al)
        output = F.dropout(h_al, self.drop_prob_lm, self.training)
        #output0 = F.dropout(h_lang, 0.5, self.training)
        #output1 = F.dropout(c_lang, 0.5, self.training)
        #h_att = h_att + output0
        #c_att = c_att + output1
        #c_att = c_att * att_gate2

        state = (torch.stack([h_att, h_lang]), torch.stack([c_att, c_lang]))
        #state = None
        return output, state

class UpDownCore_res3(nn.Module):
    def __init__(self, opt, use_maxout=False):
        super(UpDownCore_res3, self).__init__()
        self.drop_prob_lm = opt.drop_prob_lm

        self.att_lstm = nn.LSTMCell(opt.input_encoding_size + opt.rnn_size * 2, opt.rnn_size) # we, fc, h^2_t-1
        self.lang_lstm = nn.LSTMCell(opt.rnn_size * 2, opt.rnn_size) # h^1_t, \hat v
        self.att_lang = nn.LSTMCell(opt.rnn_size * 2, opt.rnn_size)
        self.att_lang2 = nn.LSTMCell(opt.rnn_size * 2, opt.rnn_size)
        self.attention = Attention(opt)
        self.attention2 = Attention(opt)
        self.attention3 = Attention(opt)
        #self.h2h_2 = nn.Linear(opt.rnn_size, opt.rnn_size)
    ...
    def forward(self, xt, fc_feats, att_feats, p_att_feats, state, att_masks=None):
        ...
        att2 = self.attention2(h_lang, att_feats, p_att_feats, att_masks)
        att_lang_input = torch.cat([att2, h_lang],1)
        h_al, c_al = self.att_lang(att_lang_input, (h_lang+h_att, c_lang+c_att))
        
        att3 = self.attention3(h_al, att_feats, p_att_feats, att_masks)
        att_lang2_input = torch.cat([att3, h_al],1)
        h_al2, c_al2 = self.att_lang(att_lang2_input, (h_lang+h_att+h_al, c_lang+c_att+c_al))
        output = F.dropout(h_al2, self.drop_prob_lm, self.training)
        #output0 = F.dropout(h_lang, self.drop_prob_lm, self.training)
        #output1 = F.dropout(c_lang, 0.5, self.training)
        #h_att = h_att + output0
        #c_att = c_att + output1
        #c_att = c_att * att_gate2
...
class NTTUD(AttModel):
    def __init__(self, opt):
        super(NTTUD, self).__init__(opt)
        self.num_layers = 2
        self.core = NTTUD_core(opt)

class NTTUD2(AttModel):
    def __init__(self, opt):
        super(NTTUD2, self).__init__(opt)
        self.num_layers = 2
        self.core = NTTUD2_core(opt)
